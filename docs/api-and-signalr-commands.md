...
| ------------------------ | --------------------- | ------------------------------ | ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `join_game_room`         | `JoinGameRoom`        | `roomCode`, `playerId?`        | `JoinGameRoomHandler`        | Subscribes the connection to the room group (and player group if `playerId` given). No ack -- and the SignalR protocol-level completion that _does_ come back fires as soon as Wolverine durably enqueues the message, not once group membership is actually added; see GAME_FLOW.md §7's "Known limitation" note (join/broadcast race). |
